            ...
            {paths.map((path, index) => (
              <React.Fragment key={index}>
                ...
                <div className="group/item relative">
                  <button
                    className={`
                      px-5 py-2.5 rounded-full text-sm font-bold tracking-tight transition-all duration-300 flex items-center gap-2
                      ${
                        index === paths.length - 1
                          ? "bg-white text-slate-900 shadow-[0_10px_20px_rgba(0,0,0,0.04)] border border-slate-50"
                  ...
                  {/* TOOLTIP: Modern Slate Design */}
                  <div className="absolute -top-12 left-1/2 -translate-x-1/2 px-3 py-1.5 bg-slate-900 text-[10px] font-bold text-white uppercase tracking-widest rounded-xl opacity-0 group-hover/item:opacity-100 transition-all -translate-y-1 group-hover/item:translate-y-0 pointer-events-none z-50">
                    Navigate
